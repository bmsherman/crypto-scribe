...
If $H$ is a 2-universal hash family on $\mathcal{X} \leftarrow \mathcal{Y}$, then for any $x \in \mathcal{X}$, $\mathcal{L} \in \mathcal{X} \rightarrow \mathcal{Y}$, $(h,h(x)) \approx (h,y)$, where $h \leftarrow H$ and $y \leftarrow_U \mathcal{Y}$.
\end{lemma}
